...
trust_store_file=""
trust_store_private_key_file="root.key"
...
if [ "$generate_trust_store" == "y" ]; then
  ...
  echo " - NOTE that the Common Name (CN) is currently not important."
    ...
    cp /path/to/your/root.key $TRUSTSTORE_WORKING_DIRECTORY/ca-key
    openssl req -new -key $TRUSTSTORE_WORKING_DIRECTORY/ca-key \
  ...
  keytool -keystore $TRUSTSTORE_WORKING_DIRECTORY/$DEFAULT_TRUSTSTORE_FILENAME \
    -alias CARoot -import -file $TRUSTSTORE_WORKING_DIRECTORY/ca-key
